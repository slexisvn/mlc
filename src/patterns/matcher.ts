...
export function matchChains(
  ...
  if (hasCycle) return []; // Guard: cannot match reliably in a cyclic graph.
  ...
  const matches: MatchedChain[] = [];

  for (const startNodeId of order) {
    if (used.has(startNodeId)) continue;

    for (const rule of sortedRules) {
      const chain = tryMatchChain(
        graph, rule, startNodeId, consumers, graphOutputSet, used,
      );
      if (chain !== null) {
        // Commit all nodes of this chain so no other rule can overlap them.
        for (const nid of chain.nodeIds) used.add(nid);
        matches.push(chain);
...
function tryMatchChain(
  ...
  for (let step = 1; step < rule.pattern.length; step++) {
    ...
    if (graphOutputSet.has(outputTid)) return null;
