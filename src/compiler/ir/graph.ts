...
export class Graph {
  ...
  /**
   * Insert a pre-built node and its associated tensors immediately BEFORE a
   * specified node in the execution order.
   * @param beforeNodeId  The node to insert before.  Falls back to prepending
   *                      if the id is not found.
   */
  _insertNodeBefore(node: Node, tensors: Tensor[], beforeNodeId: string): void {
    for (const t of tensors) this._tensors.set(t.id, t);
    this._nodes.set(node.id, node);

    const idx = this._nodeOrder.indexOf(beforeNodeId);
    if (idx >= 0) {
      this._nodeOrder.splice(idx, 0, node.id);
    } else {
