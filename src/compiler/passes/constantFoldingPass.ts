...
import { Shape, ConstantPayload }                         from "../ir/types";
...
export function resetCFCounter(): void { _cfCounter = 0; }

// ─── Numeric kernels ─────────────────────────────────────────────────────────

/** Total element count for `shape`.  Scalars (shape = []) have count 1. */
function elementCount(shape: Shape): number {
  return shape.length === 0 ? 1 : shape.reduce((acc, d) => acc * d, 1);
}

function applyUnary(op: string, x: number): number {
  switch (op) {
...
function tryFold(
  node:     Node,
  payloads: readonly ConstantPayload[],
  graph:    Graph,
  logs:     PassLog[],
): { data: number[]; shape: Shape } | null {
...
export class ConstantFoldingPass implements Pass {
  ...
  run(graph: Graph): PassResult {
    ...
    for (const nodeId of order) {
      ...
      if (!this.opRegistry.isFoldable(node.op)) continue;
      ...
      const inputPayloads = node.inputs.map(
        tid => workGraph.getTensor(tid).constantPayload,
      );
      if (inputPayloads.some(p => p === undefined)) continue;

      // ── Evaluate the operation numerically ────────────────────────────────
      const result = tryFold(node, inputPayloads as ConstantPayload[], workGraph, logs);
