...
import { Graph }                                 from "../../compiler/ir/graph";
import { DType, Shape }                          from "../../compiler/ir/types";
...
export function importGraphIR(
  ...
  const result = importSingleGraphIR(graphIR, options.graphId);

  // ── Attach constantPayload for compile-time constants ────────────────────
  // Parameters whose isConst flag is set have fully-known values; expose them
  // to the compiler so ConstantFoldingPass can fold downstream ops.
  if (pkg.parameters) {
    for (const param of pkg.parameters) {
      if (!param.isConst) continue;
      const compilerId = result.tensorIdMap.get(param.tensorId as TensorId);
      if (!compilerId) continue;
      result.graph._setConstantPayload(compilerId, { data: [...param.data] });
    }
  }
