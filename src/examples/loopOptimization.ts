...
import { createFullPipeline }       from "../passes/pipelines";

export function runLoopOptimizationExample(): void {
  resetCounters();
  ...
  const mm  = g.addNode("matmul", [x.id, w.id],
    [{ name: "mm_out", dtype: "float32", shape: [M, N] }]);
  const add = g.addNode("add", [mm.outputs[0], bias.id],
    [{ name: "add_out", dtype: "float32", shape: [M, N] }]);
  const relu = g.addNode("relu", [add.outputs[0]],
    [{ name: "out", dtype: "float32", shape: [M, N] }]);
  g.markOutputs(relu.outputs[0]);

  const original = g.clone();
  printGraph(original, "Loop Optimization — original graph (matmul + add + relu)");
  ...
  printGraph(graphOptimized, "Loop Optimization — after graph pipeline (linear_relu fused)");
  printDiff(original, graphOptimized, "Graph pipeline delta");

  const rawModule = loopPass.getLastModule()!;
  printLoopModule(rawModule, "Loop IR — after lowering (fused linear_relu nest)");
  ...
  printLoopModule(optimizedModule, "Loop IR — after loop optimization (tiled i and j dims)");
  ...
  let cur: import("../ir/loopIR").LoopStmt | undefined = fnBefore.body[0];
  while (cur && cur.kind === "ForLoop") { depthBefore++; cur = cur.body[0]; }
  cur = fnAfter.body[0];
  while (cur && cur.kind === "ForLoop") { depthAfter++;  cur = cur.body[0]; }
