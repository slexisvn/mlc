...
class Graph {
     ...
     */
    addInputTensor(name, dtype = "float32", shape = []) {
        const t = {
            id: `t${_tensorCounter++}`,
            name,
            dtype,
     ...
     */
    addNode(op, inputIds, outputSpecs, attrs = {}) {
        ...
        const outputTensors = outputSpecs.map(spec => {
            const t = {
                id: `t${_tensorCounter++}`,
                name: spec.name,
    ...
    get nodeOrder() { return this._nodeOrder; }
    get nodes() { return this._nodes; }
    get tensors() { return this._tensors; }
    get inputIds() { return this._inputs; }
    get outputIds() { return this._outputs; }
    ...
    _replaceNode(id, updated) {
        if (!this._nodes.has(id))
            throw new Error(`_replaceNode: unknown node "${id}"`);
     ...
     */
    _insertNode(node, tensors, afterNodeId) {
    ...
    _replaceOutputTensor(oldId, newId) {
     ...
     */
    _setConstantPayload(tensorId, payload) {
        const t = this._tensors.get(tensorId);
        if (!t)
            throw new Error(`_setConstantPayload: tensor "${tensorId}" not found`);
     ...
     */
    _replaceWithConstNode(nodeId, constNodeId, attrs = {}) {
        const original = this._nodes.get(nodeId);
        if (!original)
            throw new Error(`_replaceWithConstNode: node "${nodeId}" not found`);
        ...
        for (const tid of original.outputs) {
            ...
            if (t)
                this._tensors.set(tid, { ...t, producerNodeId: constNodeId });
        }
        const constNode = {
            id: constNodeId,
            op: "const",
