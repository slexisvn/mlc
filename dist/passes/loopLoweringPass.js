...
exports.LoopLoweringPass = void 0;
const loopIR_1 = require("../ir/loopIR");
// ── Pass ──────────────────────────────────────────────────────────────────────
class LoopLoweringPass {
    constructor() {
        this.name = "LoopLoweringPass";
    ...
    run(graph) {
        ...
        logs.push({
            level: "info",
            message: `Lowered ${graph.outputIds.length} output(s) → ` +
     ...
     */
    _findAncestors(graph, outputTid) {
        const outTensor = graph.getTensor(outputTid);
        if (!outTensor.producerNodeId)
            return []; // graph input — no nodes needed
        const visited = new Set();
        const queue = [outTensor.producerNodeId];
        while (queue.length > 0) {
            ...
            for (const inputTid of graph.getNode(nid).inputs) {
                ...
                if (t.producerNodeId)
                    queue.push(t.producerNodeId);
            }
        }
        // Filter graph.nodeOrder (already topo-sorted) to preserve ordering.
        return graph.nodeOrder.filter(nid => visited.has(nid));
    }
    // ── Op dispatch ───────────────────────────────────────────────────────────
    _lowerNode(node, graph) {
        switch (node.op) {
            case "add":
                return {
                    stmts: this._lowerElementwise(node, graph, ([a, b]) => (0, loopIR_1.binOp)("+", a, b)),
                };
            case "sub":
                return {
                    stmts: this._lowerElementwise(node, graph, ([a, b]) => (0, loopIR_1.binOp)("-", a, b)),
            ...
            case "relu":
                return {
                    stmts: this._lowerElementwise(node, graph, ([x]) => (0, loopIR_1.callBuiltin)("max", [(0, loopIR_1.literal)(0), x])),
                };
            case "add_relu":
                return {
                    stmts: this._lowerElementwise(node, graph, ([a, b]) => (0, loopIR_1.callBuiltin)("max", [(0, loopIR_1.literal)(0), (0, loopIR_1.binOp)("+", a, b)])),
                };
            case "matmul":
            ...
            default:
                return {
                    stmts: [],
                    diagnostic: `Node "${node.id}" (${node.op}): no lowering rule available — skipped.`,
                };
        }
    }
     ...
     */
    _lowerElementwise(node, graph, computeExpr) {
        ...
        const vars = varNames.map(n => (0, loopIR_1.loopVar)(n));
        const outRef = (0, loopIR_1.memRef)(outTensor.name, vars);
        const inputRefs = node.inputs.map(tid => (0, loopIR_1.memRef)(graph.getTensor(tid).name, vars));
        const innerBody = [(0, loopIR_1.assign)(outRef, computeExpr(inputRefs))];
        return (0, loopIR_1.nestedLoops)(varNames, outShape, innerBody);
    }
    // ── Matmul lowering (2-D) ─────────────────────────────────────────────────
    /**
    ...
    _lowerMatmul(node, graph) {
        ...
        const iV = (0, loopIR_1.loopVar)("i");
        const jV = (0, loopIR_1.loopVar)("j");
        const kV = (0, loopIR_1.loopVar)("k");
        const outIJ = (0, loopIR_1.memRef)(outName, [iV, jV]);
        const xIK = (0, loopIR_1.memRef)(xName, [iV, kV]);
        const wKJ = (0, loopIR_1.memRef)(wName, [kV, jV]);
        const kBody = [
            (0, loopIR_1.assign)(outIJ, (0, loopIR_1.binOp)("*", xIK, wKJ), true), // out[i,j] += x[i,k] * w[k,j]
        ];
        const jBody = [
            (0, loopIR_1.assign)(outIJ, (0, loopIR_1.literal)(0)), // out[i,j] = 0.0  (init)
            (0, loopIR_1.forLoop)("k", 0, K, kBody), // reduction over k
        ];
        return [(0, loopIR_1.forLoop)("i", 0, M, [(0, loopIR_1.forLoop)("j", 0, N, jBody)])];
    }
    ...
    _lowerLinearRelu(node, graph) {
        ...
        const biasRef = biasShape.length >= 2
            ? (0, loopIR_1.memRef)(biasName, [iV, jV])
            : (0, loopIR_1.memRef)(biasName, [jV]);
        const kBody = [
            (0, loopIR_1.assign)(outIJ, (0, loopIR_1.binOp)("*", xIK, wKJ), true), // out[i,j] += x[i,k]*w[k,j]
        ];
        const jBody = [
            (0, loopIR_1.assign)(outIJ, biasRef), // out[i,j]  = bias[...]
            (0, loopIR_1.forLoop)("k", 0, K, kBody), // reduction loop
            (0, loopIR_1.assign)(outIJ, (0, loopIR_1.callBuiltin)("max", [(0, loopIR_1.literal)(0), outIJ])), // relu activation
        ];
        return [(0, loopIR_1.forLoop)("i", 0, M, [(0, loopIR_1.forLoop)("j", 0, N, jBody)])];
    }
}
exports.LoopLoweringPass = LoopLoweringPass;
//# sourceMappingURL=loopLoweringPass.js.map